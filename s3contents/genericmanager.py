...
class GenericContentsManager(ContentsManager, HasTraits):
    ...
    def _directory_model_from_path(self, path, content=False):
        ...
        if self.fs.isdir(path):
            ...
            self.log.debug(f"s3_detail_to_model={s3_detail_to_model}")
            self.log.debug(f"dir_s3_detail: path='{path}', lstat={lstat}")
    ...
    def save(self, model, path):
        ...
        self.log.debug(f"save with path={path} and model={model}")
